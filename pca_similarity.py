...
def subsample_data(embeddings, uuids, sample_size):
    subsampled_uuids, subsampled_embeddings = shuffle(uuids, embeddings, n_samples=sample_size, random_state=42)
    return subsampled_uuids, np.array(subsampled_embeddings)

def apply_incremental_and_kernel_pca_and_save(embeddings_pickle_path, pca_pickle_path, pca_model_path, kernel_pca_pickle_path, n_components_ipca=200, n_components_kpca=100, batch_size=10000, kernel_pca_sample_size=10000):
    ...
    reduced_embeddings_ipca = incremental_pca_transform(embedding_matrix, ipca, batch_size)

    # Save the Incremental PCA reduced embeddings
    ipca_data = {uuid: embedding for uuid, embedding in zip(image_uuids, reduced_embeddings_ipca)}
    ...
    subsampled_uuids, subsampled_embeddings = subsample_data(reduced_embeddings_ipca, image_uuids, kernel_pca_sample_size)

    # Step 3: Apply Kernel PCA on the subsampled data
    print("Applying Kernel PCA...")
    kpca, reduced_embeddings_kpca = apply_kernel_pca(subsampled_embeddings, n_components=n_components_kpca)
    ...
    kpca_data = {uuid: embedding for uuid, embedding in zip(subsampled_uuids, reduced_embeddings_kpca)}
...
if __name__ == "__main__":
    ...
    pca_pickle_path = "pca_embeddings_image_analysis.pkl"  # Path where the Incremental PCA reduced embeddings will be saved
    pca_model_path = "pca_model_image_analysis.pkl"  # Path to save the Incremental PCA model
    ...
    apply_incremental_and_kernel_pca_and_save(
        ...
        n_components_ipca=200,  # Number of components for Incremental PCA
