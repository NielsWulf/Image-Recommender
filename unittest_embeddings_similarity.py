...
import numpy as np
from similarity_Measure_Embeddings import compute_embedding
import torch


class TestComputeEmbedding(unittest.TestCase):

    @patch("similarity_Measure_Embeddings.Image.open")
    @patch("similarity_Measure_Embeddings.preprocess")
    @patch("similarity_Measure_Embeddings.model")
    def test_compute_embedding_valid_image(
        ...
        mock_output = torch.randn(
            1, 2048
        )  # Expected output size from ResNet50 without last layer
